...
import {
  animationWorkspaceActors,
  projectFromAnimationWorkspace,
} from "../src/animation-project-export.js";
import {
...
describe("animation workspace project export", () => {
  it("merges every focused actor document into canonical project order", () => {
    ...
    expect(merged.actors.map((actor) => actor.id)).toEqual(
      studioProject.actors.map((actor) => actor.id),
    );
