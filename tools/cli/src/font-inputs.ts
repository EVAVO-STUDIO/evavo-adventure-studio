  ...
  return [
    {
      ...
      code: "schema-invalid",
      path: "$",
      message: error instanceof Error ? error.message : "Bitmap font schema validation failed.",
    },
  ];
};
...
export const loadBitmapFonts = async (
  ...
  const diagnostics: CliDiagnostic[] = validateBitmapFontManifest(project, manifest).map((issue) => ({
...
export const bitmapFontInputPaths = (loaded: LoadedBitmapFonts): readonly string[] =>
