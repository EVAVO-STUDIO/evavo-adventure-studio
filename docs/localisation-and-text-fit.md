...
Save-game bundle fingerprints now use a localisation-neutral compatibility view:
...
The governed Editor Expansion command includes these tests, strict TypeScript compilation and the Player production build. A successful source or type pass does not replace 1× native visual review with the final bitmap atlas and translated content.
