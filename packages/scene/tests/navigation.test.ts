...
const area = (areaId: string, points: readonly Point[]): NavigationArea => ({
...
describe("deterministic navigation routes", () => {
  it("routes around the reflex corner of a concave walk polygon", () => {
    ...
    const result = findNavigationRoute(start, end, [area("navigation.concave", concave.points)]);
    expect(result.kind).toBe("route");
    if (result.kind !== "route") {
      throw new Error("Expected a route through the concave room.");
    }

    expect(result.route.points).toEqual([start, { x: 40, y: 40 }, end]);
    expect(
      result.route.segments.every((segment) => segmentInsidePolygon(segment.from, segment.to, concave)),
    ).toBe(true);
  });

  it("snaps an outside click to the nearest reachable boundary", () => {
    const result = findNavigationRoute({ x: 10, y: 25 }, { x: 70, y: 25 }, [
  ...
  it("crosses separate navigation surfaces only through authored portals", () => {
    ...
    const disconnected = findNavigationRoute({ x: 10, y: 25 }, { x: 140, y: 25 }, [left, right], []);
    expect(disconnected).toEqual({
      kind: "unreachable",
      reason: "no-connected-route",
    });

    const connected = findNavigationRoute({ x: 10, y: 25 }, { x: 140, y: 25 }, [left, right], [portal]);
  ...
  it("chooses the lexically stable portal when equal routes exist", () => {
    ...
    const portals: readonly NavigationPortal[] = ["portal.b", "portal.a"].map((portalId) => ({
    ...
    const result = findNavigationRoute({ x: 10, y: 25 }, { x: 140, y: 25 }, [left, right], portals);

    expect(result.kind).toBe("route");
    if (result.kind === "route") {
      expect(result.route.segments.find((segment) => segment.kind === "portal")?.portalId).toBe("portal.a");
    }
  });
