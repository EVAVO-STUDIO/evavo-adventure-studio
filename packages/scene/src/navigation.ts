...
const uniqueSorted = (values: readonly number[]): readonly number[] => {
  ...
  for (const value of sorted) {
    if (
      result.length === 0 ||
      Math.abs(value - (result[result.length - 1] ?? 0)) > EPSILON
    ) {
      result.push(value);
    }
  }
...
const addUndirectedWalkEdge = (
  ...
  edges.get(left.key)?.push(leftEdge);
  edges.get(right.key)?.push(rightEdge);
};
...
const addPortalEdge = (
  ...
  edges.get(from.key)?.push({
    fromKey: from.key,
    toKey: to.key,
    cost: distance(from.point, to.point) + (portal.traversalCost ?? 0),
    kind: "portal",
    areaId: null,
    portalId: portal.id,
  });
...
const buildGraph = (
  ...
  const portalNodes = new Map<
    string,
    { readonly from: GraphNode; readonly to: GraphNode; readonly portal: NavigationPortal }
  >();
  for (const portal of [...portals].sort((left, right) =>
    ...
    addNode(from);
    addNode(to);
    portalNodes.set(portal.id, { from, to, portal });
  }
    ...
    for (let leftIndex = 0; leftIndex < areaNodes.length; leftIndex += 1) {
      for (
        ...
        if (
          ...
          addUndirectedWalkEdge(edges, left, right);
        }
      }
    }
  }

  for (const { from, to, portal } of portalNodes.values()) {
    addPortalEdge(edges, from, to, portal);
    if (portal.bidirectional) {
      addPortalEdge(edges, to, from, portal);
    }
  }
...
const solveGraph = (
...
): { readonly endKey: string; readonly previous: ReadonlyMap<string, PreviousStep> } | null => {
  ...
  while (pending.size > 0) {
    ...
    if (endKeys.has(currentKey)) {
      return { endKey: currentKey, previous };
    }
...
const reconstructRoute = (
  nodes: ReadonlyMap<string, GraphNode>,
  solution: { readonly endKey: string; readonly previous: ReadonlyMap<string, PreviousStep> },
  ...
  for (const segment of segments) {
    if (
      points.length === 0 ||
      squaredDistance(points[points.length - 1] ?? segment.from, segment.from) >
        EPSILON
    ) {
    ...
    if (
      points.length === 0 ||
      squaredDistance(points[points.length - 1] ?? segment.to, segment.to) >
        EPSILON
    ) {
...
export const findNavigationRoute = (
  ...
  const firstStartKey = reconstructed.segments[0]?.from;
  const lastEndKey = reconstructed.segments.at(-1)?.to;
  const startAreaId =
    graph.nodes.get(
      reconstructed.segments[0]?.from
        ? reconstructed.segments[0].kind === "walk"
          ? reconstructed.segments[0].areaId
            ? `start:${reconstructed.segments[0].areaId}`
            : graph.startKeys[0] ?? ""
          : graph.startKeys[0] ?? ""
        : graph.startKeys[0] ?? "",
    )?.areaId ?? start.areaIds[0];
  const endNode = graph.nodes.get(solution.endKey);
  const endAreaId = endNode?.areaId ?? end.areaIds[0];
  if (!startAreaId || !endAreaId || !firstStartKey || !lastEndKey) {
    if (squaredDistance(start.point, end.point) <= EPSILON) {
      const areaId = start.areaIds[0];
      if (!areaId) {
        return { kind: "unreachable", reason: "no-connected-route" };
      }
      return {
