...
export const selectApproachSlot = (
  ...
  const candidates = slots
    ...
    .filter(
      (slot) =>
        !request.itemId || slot.validItemIds.length === 0 || slot.validItemIds.includes(request.itemId as never),
    )
    .filter((slot) => !request.reachable || request.reachable(slot.position))
    .map((slot) => ({
      slot,
      distanceSquared: squaredDistance(request.actorPosition, slot.position),
    }))
    .sort((left, right) => {
  ...
  return selected
    ? { slot: selected.slot, distance: Math.sqrt(selected.distanceSquared) }
    : null;
};
