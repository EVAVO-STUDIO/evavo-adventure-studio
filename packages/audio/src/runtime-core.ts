...
const cueForId = (
  ...
  if (!cue) throw new Error(`Audio cue '${cueId}' does not exist.`);
  return cue;
};
...
const configuredBus = (
  ...
  if (!configured) throw new Error(`Audio bus '${bus}' is not configured.`);
  return configured;
};
...
const effectiveBusVolume = (
  ...
  for (const rule of manifest.ducking) {
    ...
    if (
      ...
      multiplier = Math.min(multiplier, rule.targetVolume);
    }
  }
  return runtime.volume * multiplier;
};
...
const duckingFadeTicks = (
  ...
  for (const rule of manifest.ducking) {
    ...
    if (!wasActive && isActive) result = Math.max(result, rule.attackTicks);
    if (wasActive && !isActive) result = Math.max(result, rule.releaseTicks);
  }
  return result;
};
...
const appendChangedBusCommands = (
  ...
  ignoredBuses: ReadonlySet<AudioBusId> = new Set(),
): void => {
  for (const bus of audioBusIds) {
    if (ignoredBuses.has(bus)) continue;
...
const transitionWithDucking = (
  ...
  before: AudioRuntimeState,
  after: AudioRuntimeState,
  initialCommands: readonly AudioCommand[],
  ignoredBuses: ReadonlySet<AudioBusId> = new Set(),
): AudioRuntimeTransition => {
  const commands = [...initialCommands];
  appendChangedBusCommands(
    manifest,
    before,
    after,
    commands,
  ...
  return { state: after, commands: sortCommands(commands) };
};
...
export const audioVoicePlaybackOffsetMilliseconds = (
  ...
  const length = voice.loop.endMilliseconds - voice.loop.startMilliseconds;
  return length <= 0
    ? voice.loop.startMilliseconds
    : voice.loop.startMilliseconds +
        ((rawOffset - voice.loop.startMilliseconds) % length);
};
...
const enforceVoiceLimits = (
  ...
  const bus = configuredBus(manifest, protectedVoice.bus);
  const busVoices = voices.filter(
    (voice) => voice.bus === protectedVoice.bus,
  );
  const excess = Math.max(0, busVoices.length - bus.maxVoices);
  return excess === 0
    ? []
    : [...busVoices]
...
export const triggerAudioCue = (
  ...
  if (cue.interruptGroup) {
    const interrupted = voices.filter(
      (voice) => voice.interruptGroup === cue.interruptGroup,
    );
    commands.push(...stopCommandsFor(interrupted, atTick, cue.fadeOutTicks));
    voices = [
      ...removeVoices(
        voices,
        new Set(interrupted.map((voice) => voice.id)),
      ),
    ];
  }
  ...
  if (cue.polyphony === "restart" && matching.length > 0) {
    ...
    voices = [
      ...removeVoices(voices, new Set(matching.map((voice) => voice.id))),
    ];
  }
  if (cue.polyphony === "overlap" && matching.length >= cue.maxInstances) {
    const stolen = [...matching]
      .sort(voiceStealOrder(manifest, cue.bus))
      .slice(0, matching.length - cue.maxInstances + 1);
    commands.push(...stopCommandsFor(stolen, atTick, cue.fadeOutTicks));
    voices = [
      ...removeVoices(voices, new Set(stolen.map((voice) => voice.id))),
    ];
  }
  ...
  const voice = createCueVoice(
    cue,
    voiceId(`audio-voice.cue.${cue.id}.${serial}`),
    atTick,
  ...
  const busSteals = enforceVoiceLimits(manifest, voices, voice.id);
  commands.push(...stopCommandsFor(busSteals, atTick, cue.fadeOutTicks));
  voices = [
    ...removeVoices(voices, new Set(busSteals.map((candidate) => candidate.id))),
  ];
  commands.push({
    kind: "play",
    atTick,
...
export const enterAudioScene = (
  ...
  for (const layer of desired) {
    ...
    const reusable = existingSceneVoices
      .filter(
        (voice) => voice.cueId === layer.cueId && !retained.has(voice.id),
      )
...
export const advanceAudioRuntimeState = (
  ...
  return transitionWithDucking(manifest, state, { ...state, tick }, []);
};
