...
import { advanceFixedStepClock, createFixedStepClock, type FixedStepConfig } from "../src/fixed-step.js";
...
describe("fixed-step clock", () => {
  it("accumulates fractional host time without inventing logical ticks", () => {
    const first = advanceFixedStepClock(createFixedStepClock(), 1000 / 120, config);
    expect(first.ticksToRun).toBe(0);
    expect(first.interpolationAlpha).toBeCloseTo(0.5);

    const second = advanceFixedStepClock(first.state, 1000 / 120, config);
  ...
  it("bounds catch-up and drops excess whole ticks", () => {
    const advanced = advanceFixedStepClock(createFixedStepClock(), 100, config);
  ...
  it("clamps very large host stalls before catch-up", () => {
    const advanced = advanceFixedStepClock(createFixedStepClock(), 2000, config);

    expect(advanced.ticksToRun).toBe(4);
    expect(advanced.droppedMilliseconds).toBeGreaterThan(1800);
    expect(advanced.state.totalDroppedMilliseconds).toBe(advanced.droppedMilliseconds);
  });

  it("rejects invalid clock input instead of corrupting time", () => {
    expect(() => advanceFixedStepClock(createFixedStepClock(), -1, config)).toThrow(RangeError);
    expect(() =>
